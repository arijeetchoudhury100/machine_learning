...
def bayesian_classifier(x,cov,mean,p):
	inv_cov = np.linalg.inv(cov)
	det_cov = np.linalg.det(cov)
	g = -0.5*((x-mean).dot(inv_cov).dot((x-mean).transpose())) - 0.5*np.log(det_cov) + np.log(p)
	return g
...
for i in range(0,len(iris_data_X_test)):
	p0 = bayesian_classifier(np.array([iris_data_X_test[i]]),cov1,m1,prior1)
	p1 = bayesian_classifier(np.array([iris_data_X_test[i]]),cov2,m2,prior2)
	p2 = bayesian_classifier(np.array([iris_data_X_test[i]]),cov3,m3,prior3)

	r0 = loss[0][0]*p0 + loss[0][1]*p1 + loss[0][2]*p2 
	r1 = loss[1][0]*p0 + loss[1][1]*p1 + loss[1][2]*p2
	r2 = loss[2][0]*p0 + loss[2][1]*p1 + loss[2][2]*p2
...
for i in range(0,len(predictions)):
	if predictions[i] == iris_data_y_test[i]:
		correct = correct + 1
print('Accuracy min risk:',(correct/len(predictions))*100)

predictions = []
for i in range(0,len(iris_data_X_test)):
	g1 = bayesian_classifier(np.array([iris_data_X_test[i]]),cov1,m1,prior1)
	g2 = bayesian_classifier(np.array([iris_data_X_test[i]]),cov2,m2,prior2)
	g3 = bayesian_classifier(np.array([iris_data_X_test[i]]),cov3,m3,prior3)
...
for i in range(0,len(predictions)):
	if predictions[i] == iris_data_y_test[i]:
		correct = correct + 1
print('Accuracy bayesian:',(correct/len(predictions))*100)