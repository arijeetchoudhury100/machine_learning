...
def mahalanobis_distance(a,b,cov):
	...
	a = np.delete(a,3,1)
	dis = (a-b).dot(cov_inv).dot((a-b).transpose())
	return dis[0][0];
...
#classify test points using mahalanobis distance
predictions = []
for i in range(0,3000):
	d1 = mahalanobis_distance(np.array([Xt[i]]),m1_estd,cov1_estd)
	d2 = mahalanobis_distance(np.array([Xt[i]]),m2_estd,cov2_estd)
	d3 = mahalanobis_distance(np.array([Xt[i]]),m3_estd,cov3_estd)
	if d1<d2 and d1<d3:
